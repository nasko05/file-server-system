...
mod tests {
    ...
    async fn test_delete_user_directory_success() {
        ...
        // temp_dir is dropped here, cleaning up the temporary root.
    }
    ...
    async fn test_delete_file_success() {
        ...
        // For files, the endpoint concatenates the file path and filename.
        // It expects payload.path to include a trailing slash.
        let sub_path = "files/";
        let user_dir = test_root.join(username);
        let files_dir = user_dir.join(sub_path);
        fs::create_dir_all(&files_dir).expect("failed to create files directory");

        // Create a file at: ROOT_DIR/test_user/files/delete_me.txt
    ...
    async fn test_delete_file_not_a_file() {
        ...
        // Create a directory with the name instead of a file.
